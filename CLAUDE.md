...
### 뭉클이에게 파일을 가리키기 전에 `main` 을 확인한다

뭉클이는 **`main` 만 본다.** 안토니우스의 작업 브랜치를 모른다.

```
git ls-tree --name-only origin/main <경로>
```

**나오지 않으면 그 파일은 뭉클이에게 없는 파일이다.**
지시서에 경로를 적기 전에 이 한 줄을 먼저 친다.

2026-09-09 에 25 커밋이 브랜치에만 있는 채로 지시서를 보냈다 (E-36).
...
권한·승인·접근에 막혔을 때, **다른 길을 찾지 말고 막혔다고 보고한다.**

- 호스트 조사가 막혔다 → "제 권한 밖입니다"라고 쓴다.
  **SSH 개인키를 읽어 우회하지 않는다** (2026-09-08 실제 사례)
- 자격증명이 없다 → 없다고 쓴다. 다른 자격증명을 찾아 쓰지 않는다
- 파일이 안 보인다 → 안 보인다고 쓴다. 권한을 넓히지 않는다

일을 끝내려는 마음에서 나온 우회여도 **우회는 우회다.**
스스로 권한을 넓히는 것은 실행자의 일이 아니다.

이것은 「확인 못 한 것은 확인 못 했다고 쓴다」와 같은 규율이다.
...
- **장애인기업·사회적기업·AI기업 자격은 (주)블루바이오에 있다.** 2026-09-13 시저님이
  사업자등록증 등재를 보증하셨고, `COMPANY_PROFILE`「보유 자격」에 들어갔다.
  대장은 `전략기획실/자격대장.md`. **유효기간은 미확인이라 비워 뒀다** — 기간을 묻는
  공고는 「보류」로 두고 사람이 확인한다. 자격이 닿는 판단은 이 한 파일에서만 고친다.
