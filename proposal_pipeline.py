...
def search_supply_prices(items: list[dict]) -> list[dict]:
    ...
    for item in items:
        ...
        suppliers = price_info.get("suppliers", [])

        # 업체 목록 출력
        if suppliers:
            for s in suppliers:
                print(f"    └ {s['mallName']}: {s['price']:,}원 ({s['title'][:25]})")
        ...
        results.append({
            ...
            "공급업체목록": suppliers,  # [{mallName, price, title, link}, ...]
        })
    return results
...
def generate_proposal_with_prices(bid_info: dict, price_data: list[dict]) -> str:
    ...
    for p in price_data:
        if p["도매참고가"]:
            price_text += f"- {p['품목명']}: 도매참고가 {p['도매참고가']:,}원 → 입찰단가 {p['입찰단가']:,}원\n"
            for s in p.get("공급업체목록", []):
                price_text += f"    공급업체: {s['mallName']} / {s['price']:,}원 / {s['title']}\n"
        else:
...
def save_proposal(proposal_text: str, bid_info: dict, price_data: list[dict], output_dir: str) -> str:
    ...
    for p in price_data:
        if p["도매참고가"]:
            ...
            for s in p.get("공급업체목록", []):
                price_summary_lines.append(
                    f"      • {s['mallName']}: {s['price']:,}원 ({s['title']})"
                )
        else:
            price_summary_lines.append(f"  - {p['품목명']}: 수동 확인 필요")
